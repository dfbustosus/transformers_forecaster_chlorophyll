...
def build_satellite_matchup_audit(config: dict[str, Any]) -> dict[str, Path]:
    ...
    report_path.write_text(
        ...
        "Figures 4–7 are now regenerated from cached TimesFM and Chronos Large "
        "rolling-origin predictions produced from the local processed Chl-a series. "
        "The cache is stored at data/processed/foundation_model_predictions.csv. "
        "Chronos Large was executed on MPS with 16 samples for tractable local inference; "
        "this setting should be disclosed for uncertainty-interval interpretation.\n\n"
